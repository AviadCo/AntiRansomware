...
bool Crypt::encryptFile(std::wstring SourceFile, std::wstring DestinationFile, std::wstring Password)
{
	
	LPCUWSTR pszSourceFile = SourceFile.c_str();
	LPCUWSTR pszDestinationFile = DestinationFile.c_str();
	LPCUWSTR pszPassword = Password.c_str();
	...
	else
	{
		...
		{
			_tprintf(
				TEXT("A cryptographic provider has been created. \n"));
		}
		else
		{
			MyHandleError(
...
void Crypt::MyHandleError(LPTSTR psz, int nErrorNumber)
{
